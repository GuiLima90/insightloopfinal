...
# Contagem por tag / por conversa
raw_counts = Conversation
  .joins(:classification)
  .group("classifications.tag")
  .count

# Descobrir quais tagas foram elegidas pelo pareto
if raw_counts.any?
  sorted = raw_counts.sort_by { |_tag, count| -count } # maior → menor

  total   = raw_counts.values.sum.to_f
  running = 0.0
  pareto_tags = []

  sorted.each do |tag, count|
    running += count
    pareto_tags << tag
    break if (running / total * 100.0) >= 80
  end

  # Pesos proporcionais pra tendencia fazer sentido sempre
  main_weights  = raw_counts.slice(*pareto_tags)      # tags do Pareto
  other_weights = raw_counts.except(*pareto_tags)     # tags restantes

  # Gerar histórico de 30 dias, sendo q dia 1 foi há 30d atrás e dia 30 é hoje
  (0..29).each do |i|
    day        = (29 - i).days.ago.to_date
    factor     = i.to_f / 29 # 0.0 → 1.0 (cresce ao longo dos dias)

    # total diário para tags principais (Pareto) – curva mais forte
    main_total_for_day  = (10 + 20 * factor).round   # ~10 → ~30
    # total diário para tags secundárias – curva mais suave
    other_total_for_day = (3  +  3 * factor).round   # ~3 → ~6

    # ----- PRINCIPAIS (tags do Pareto) -----
    main_weight_sum = main_weights.values.sum.to_f.nonzero? || 1.0

    main_weights.each do |tag, weight|
      classification = classifications_by_tag[tag]
      next unless classification

      # distribui o total do dia proporcional ao peso de cada tag
      count_for_tag = ((weight / main_weight_sum) * main_total_for_day).round

      count_for_tag.times do |j|
        Conversation.create!(
          user: user,
          classification: classification,
          channel: "Whatsapp",
          content: "[Seed tendência] #{tag} - dia sintético (##{j + 1})",
          created_at: day.to_time + rand(0..23).hours + rand(0..59).minutes
        )
      end
    end

    # ----- SECUNDÁRIAS (demais tags) -----
    other_weight_sum = other_weights.values.sum.to_f.nonzero? || 1.0

    other_weights.each do |tag, weight|
      classification = classifications_by_tag[tag]
      next unless classification

      count_for_tag = ((weight / other_weight_sum) * other_total_for_day).round
      ...
      count_for_tag.times do |j|
        Conversation.create!(
          ...
          classification: classification,
          channel: "Instagram",
          content: "[Seed estável] #{tag} - dia sintético (##{j + 1})",
          created_at: day.to_time + rand(0..23).hours + rand(0..59).minutes
        )
      end
    end
